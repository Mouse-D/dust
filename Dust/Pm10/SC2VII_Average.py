...
def aveDaily(AOTpathRe,AOTpathDa):
    ...
    while i < len(AOTRefiles):
        ...
        for j in range(step + 1):
            [lon, lat, aot] = readNetcdf4(AOTpathRe + '/' + AOTRefiles[i + j], 'lon', 'lat', 'pm10')
        ...
        writeNetcdf4(AOTpathDa + '/' + AOTRefiles[i][0:8] + 'xxxx_pm10_all.nc', 1, lon=dataOri[0][0], lat=dataOri[0][1], pm10=sum)
        i = i + step + 1
"""
...
def aveDaily(AOTpathRe,AOTpathDa):
    ...
    for i in range(len(AOTRefiles)):#2021010100xx_pm2.5
        if AOTRefiles[i][8:10]!="xx" and AOTRefiles[i][13:17]=="pm10" and AOTRefiles[i][-3:]==".nc":
            [lon, lat, aot] = readNetcdf4(AOTpathRe + '/' + AOTRefiles[i], 'lon', 'lat', 'pm10')
    ...
    if len(pmlist)>0:
        writeNetcdf4(AOTpathDa + '/' + AOTpathRe[-8:] + 'xxxx_pm10_all.nc', 1, lon=pmlist[0][0],
                 lat=pmlist[0][1], pm10=sum)
...
def aveMonthly(AOTpathDa,AOTpathMo):
    ...
    while i < len(AOTDafiles):
        ...
        cnt = np.zeros((d1, d2))
        for j in range(step + 1):
            [lon, lat, aot] = readNetcdf4(AOTpathDa + '/' + AOTDafiles[i + j], 'lon', 'lat', 'pm10')
            dataOri.append([lon, lat, aot])
            aot[np.isnan(aot)] = 10000
            aot[aot == 0] = 1000000
            aot = np.array(aot)
        ...
        writeNetcdf4(AOTpathMo + '/' + AOTDafiles[i][0:6] + 'xxxxxx_pm10_all.nc', 1, lon=dataOri[0][0], lat=dataOri[0][1], pm10=sum)
        i = i + step + 1
